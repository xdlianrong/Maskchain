...
void test_exp() {
    ...
    Elgamal_2apk_3v<FieldT> elg(g_c);
...
void test_exp_gadget() {
    ...
    ************************************************/
    ...
    elgamal_gadget<FieldT> elg_gadget(pb,g,  //生成元
                        Gpk,        
...
void test_tree()
{
    ...
    protoboard<FieldT> pb;
    //digest_variable<FieldT> leaf(pb, SHA256_digest_size, "left");
    //digest_variable<FieldT> root(pb, SHA256_digest_size, "left");
...
private:
    ...
    std::shared_ptr<digest_variable<FieldT>> root;//公钥
...
public:
    joinsplit_gadget_z(protoboard<FieldT> &pb,FieldT &sg,FieldT &gpk) : gadget<FieldT>(pb),g(sg),Gpk(gpk) {
        ...
        tree_gad_cm.reset(new tree_gadget<FieldT>(pb,root,commitment_old,""));
        cout<<"tree"<<endl;
    ...
    void generate_r1cs_witness(
        ...
        root->bits.fill_with_bits(this->pb, uint256_to_bool_vector(rt));
...
public:
    joinsplit_gadget_l(protoboard<FieldT> &pb,FieldT &sg,FieldT &gpk) : gadget<FieldT>(pb),g(sg),Gpk(gpk) {
        // pb.set_input_sizes(2);
...
void test_js_l() {
    ...
    MerkleTreePath path=getMerkleTreePath_lb();
    uint256 rt=path.root;
    /*
    void generate_r1cs_witness(
        uint256& ask,
...
private:
    ...
    std::shared_ptr<digest_variable<FieldT>> root1;//公钥

    std::shared_ptr<tree_gadget<FieldT>> tree_gad_cm2;//承诺树
    std::shared_ptr<digest_variable<FieldT>> root2;//公钥
...
public:
    joinsplit_gadget_hb(protoboard<FieldT> &pb,FieldT &sg,FieldT &gpk) : gadget<FieldT>(pb),g(sg),Gpk(gpk) {
        // pb.set_input_sizes(2);
        ...
        tree_gad_cm1.reset(new tree_gadget<FieldT>(pb,root1,commitment_old1,""));

        root2.reset(new digest_variable<FieldT>(pb, 256, ""));
        tree_gad_cm2.reset(new tree_gadget<FieldT>(pb,root2,commitment_old2,""));
        cout<<"tree"<<endl;
    ...
    void generate_r1cs_witness(
        ...
        root1->bits.fill_with_bits(this->pb, uint256_to_bool_vector(rt1));
        ...
        root2->bits.fill_with_bits(this->pb, uint256_to_bool_vector(rt2));
