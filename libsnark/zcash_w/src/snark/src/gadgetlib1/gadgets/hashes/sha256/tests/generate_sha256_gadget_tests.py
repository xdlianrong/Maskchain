...
def generate_sha256_gadget_tests():
    ...
    hexfmt = '%08x'
    print 'left:'+''.join(hexfmt % x for x in bytes_to_words(left))
    print 'right:'+''.join(hexfmt % x for x in bytes_to_words(right))
    print 'hash:'+''.join(hexfmt % x for x in bytes_to_words(hash))
