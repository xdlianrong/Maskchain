...
//接受来自受捐者的捐款请求
void acceptDonateReq()
{
    int donateReq; //存储收到的捐款请求金额
    ...
    acceptaddr.sin_port = htons(addr.address.at(0).port); 
    if( bind(acceptfd, (struct sockaddr*) &acceptaddr, sizeof(acceptaddr))== -1 )  
    {  
        cout<<" bind error! "<<strerror(errno)<<errno<<endl;  
        exit(1);      
    }  
    ...
    {  
        cout<<" listen error! "<<strerror(errno)<<errno<<endl;     
        exit(1);  
    }  
    cout<<" listen......"<<endl;  
    ...
    for(;;)
    {
        ...
        {  
            cout<<" accept error! "<<strerror(errno)<<errno<<endl;
       	    exit(1);
        }  
        else
        {
            bzero(&donateReq,sizeof(donateReq));
            if(recv(connfd,&donateReq,sizeof(donateReq),0) == -1) 
            {
                 cout<<" recv error! "<<strerror(errno)<<errno<<endl;  
                 exit(1); 
            }
            cout<<" Have accepted donation require!"<<endl;
            cout<<" Amount of donation: "<<donateReq<<endl;
...
//对所有捐款者上传的部分和求总和，并且与受捐者的请求金额作比较，看是否满足需求；

void compareDonateAmount(reqAmount)
{
    //开始从文件读取sum，进行求和
    //由于每个捐赠者都将自己的部分和及其证明写入了不同的文件，所以需要逐个文件读取
    FieldT donate_sum=0;
    FieldT sum=0;
    for(int i=0;i<D;i++)
    {
        struct secret_sum_and_prove<FieldT> buf;
    ...
    bigint_r big_sum,big_reqAmount;
    unsigned char *buf_sum;
    unsigned char *buf_reqAmount;
    fpToByte(sum,buf_sum);
    byteToBigint(big_sum,buf_sum);
    fpToByte(reqAmount,buf_reqAmount);
    byteToBigint(big_reqAmount,buf_reqAmount);
    if(big_sum>big_reqAmount)
    {
...
int main(void)
{
    ...
    FieldT reqAmount=14;
    //compareDonateAmount<FieldT>(reqAmount);
