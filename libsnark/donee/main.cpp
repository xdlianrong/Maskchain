...
void initDonateReq()
{
    cout<<"--------------Begin to initiate a request for donatation-------------- "<<endl;
    cout<<endl;
    cout<<"Please enter the donated amount you want:";
    unsigned int donateAmount;
    cin>>donateAmount;
   //向捐赠平台发送捐款请求
    int sendfd;
    sendfd = socket(AF_INET, SOCK_STREAM, 0);   
    struct sockaddr_in plataddr;  
   	bzero(&plataddr, sizeof(plataddr));

    //得到捐赠平台的ip和端口号
    getAddress addr;
    addr.getAddr(SYSTEMIPLIST);
   	string IP=addr.address.at(0).ip;  //ip列表中第一个地址为捐赠平台的，第二个地址为矿工，第三个地址为受捐人的
    char *ips=&IP[0];  
   	uint16_t port=addr.address.at(0).port;
      
    // 初始化捐赠平台套接字的 sockaddr_in
   	plataddr.sin_family = AF_INET;
    plataddr.sin_port = htons(port);  
   	if(inet_aton(ips, &plataddr.sin_addr) <= 0)
   	{
    ...
    //开始与平台进行连接
   	if(connect(sendfd, (struct sockaddr*)& plataddr, sizeof(plataddr)) < 0)
    {   
      	cout<<"connect error!"<<endl;
       	exit(0);
    }
    cout<<"Begin to send message to platform..."<<endl;
    //开始发送捐款请求
    char sendline[10];  //存储请求金额
    fgets(sendline,10,stdin);
    if (send(sendfd,&sendline,10, 0)<0)  
    {  
        cout<<"send error!"<<endl;  
        exit(1);  
    } 
    else cout<<"send to platform success!"<<endl;  
}

//向捐赠平台回馈礼物
void sendReward()
{
    cout<<"-------------Begin to send reward to platform------------"<<endl;
    cout<<"Please enter your aomunt of reward: ";
    unsigned int reward;
    cin>>reward;
    //开始给捐赠平台发送奖励金额;
    int sendfd;
    ...
    getAddress addr;
    addr.getAddr(SYSTEMIPLIST);
   	string IP=addr.address.at(0).ip;  //ip列表中第一个地址为捐赠平台的，第二个地址为受捐人，第三个地址为旷工的
    char *ips=&IP[0];  
   	uint16_t port=addr.address.at(0).port;
    ...
    //开始发送奖励
    char sendline[10];   //sendline存放奖励金额
    fgets(sendline,10,stdin);
    cout<<"Begin to send message to platform..."<<endl;
    if (send(sendfd,&sendline,10, 0)<0)  
    {  
...
int main(void)
{
    while(1)
    {
        ...
        cout<<"1.require donatation "endl;
        cout<<"2.send reward" <<endl;
        cout<<"Please enter your order number:  ";
        int order;
        cin>>order;
        if(order==1)
        {
        ...
        {
            cout<<"Order error!Please enter right order!"<<endl;
            continue;
        }
    }
