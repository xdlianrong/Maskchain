...
BlockHeader TestBlock::constructBlock(mObject const& _o, h256 const& _stateRoot)
{
    ...
    {
        BOOST_ERROR(TestOutputHelper::get().testName() + " Failed block population with Exception: "
                    << _e.what());
    }
    catch (...)
    {
...
void TestBlock::verify(TestBlockChain const& _bc) const
{
    ...
    catch (Exception const& _e)
    {
        ...
        if ((m_blockHeader.number() >= daoHardfork && m_blockHeader.number() <= daoHardfork + 9) ||
